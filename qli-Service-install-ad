...
if [ ${#token} -ge 61 ]; then
  echo "{\"Settings\":{\"baseUrl\": \"https://mine.qubic.li/\",\"amountOfThreads\": $threads,\"alias\": \"$minerAlias\",\"accessToken\": \"$token\", \"autoupdateEnabled\": false, \"allowHwInfoCollect\": true, \"overwrites\": {\"CUDA\": \"12\"}}}" > $path/$settingsFile;
else
